...
params = [
    "高压侧温度[℃]", "低压侧温度[℃]", "压缩机转速[RPM]", "E_steady[Wh/d]", "Standby[Wh/d]","化霜增量[Wh/d]","Es[Wh/d]"
]
...
def appliance_input_area(area_name, key_prefix):
    ...
    if key_prefix == "new":
        filtered_params = [p for p in params if not p.startswith("高压侧") and not p.startswith("低压侧") and not p.startswith("E_steady")]
